from Agents.default_agent_connect4 import default_agent as default_connect4
import numpy as np
import random
...
class QLearningAgent:
    ...
    def init_QTable(self, board):
        ...
        if state not in self.q_table:
            ...
            self.q_table[state] = {move: 0.0 for move in possible_moves}
        return state
    ...
    def choose_action(self, board):
        if random.uniform(0, 1) < self.epsilon:
            return default_connect4(board, board.turn)
        else:
            current_state = self.init_QTable(board)
